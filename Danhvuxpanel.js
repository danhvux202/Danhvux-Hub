// ==UserScript==
// @name         Danhvux Panel (Bot Connected)
// @version      12.0.0
// @match        *://k12online.vn/*
// @match        *://*.k12online.vn/*
...
(function() {
    ...
    // ĐỊA CHỈ BOT PYTHON CỦA BẠN
    // Nếu chạy cùng máy tính: http://localhost:5000/blacklist
    // Nếu chạy trên máy khác/VPS: thay localhost bằng IP của máy đó
    const BOT_API_URL = 'http://localhost:5000/blacklist';

    async function checkSecurity() {
        // 1. Lấy IP của người dùng hiện tại
        ...
        // 2. Gửi yêu cầu tới Bot Python để lấy danh sách cấm
        GM_xmlhttpRequest({
            method: "GET",
            url: BOT_API_URL,
            onload: function(response) {
                try {
                    const bannedIPs = JSON.parse(response.responseText);
                    
                    // 3. Nếu IP nằm trong danh sách cấm của Bot
                    if (bannedIPs.includes(userIP)) {
                        document.documentElement.innerHTML = `
                            <body style="background:#000; color:red; display:flex; align-items:center; justify-content:center; height:100vh; font-family:sans-serif; margin:0;">
                                <div style="text-align:center; border:2px solid red; padding:50px; border-radius:20px; background:rgba(255,0,0,0.1);">
                                    <h1 style="font-size:50px; margin:0;">🚫 TRUY CẬP BỊ TỪ CHỐI</h1>
                                    <p style="font-size:20px; color:#ccc;">IP: <b>${userIP}</b> đã bị Bot Danhvux cấm vĩnh viễn.</p>
                                    <p style="color:#555;">Vui lòng liên hệ Admin Discord để gỡ cấm.</p>
                                </div>
                            </body>`;
                        window.stop(); // Dừng tải trang ngay lập tức
                    }
                } catch (e) {
                    console.log("⚠️ Không thể kết nối với Bot Python. Panel chạy chế độ offline.");
                }
            },
            onerror: function() {
                console.log("❌ Bot Python chưa bật hoặc lỗi kết nối.");
            }
        });
    }
