// ==UserScript==
// @name         K12 Helper Pro - Danhvux Port 8000 (Perfect Apps)
// @namespace    http://tampermonkey.net/
// @version      21.10
// @description  Panel đẹp hoàn hảo + Apps tên chuẩn + Turbo x20
// @author       Danhvux
// @match        *://*.k12online.vn/*
...
(function() {
    ...
    const showToast = (message, type = 'info', duration = 3000) => {
        ...
        toast.style.cssText = `
            min-width: 280px; max-width: 360px; padding: 16px 20px;
            background: ${isDark ? '#1e2227' : '#ffffff'}; color: ${isDark ? '#ffffff' : '#1e2227'};
            border-radius: 12px; box-shadow: 0 8px 24px ${isDark ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.15)'};
    ...
    startSecuritySystem().then(accessGranted => {
        ...
        const updateCSS = () => {
            ...
            style.innerText = `
                ...
                #dv-panel {
                    position: fixed; top: 50px; right: 20px; width: var(--w) !important; height: auto !important;
                    background: var(--bg); color: var(--text); border-radius: 12px; font-family: 'Segoe UI', sans-serif; 
                    z-index: 100000; box-shadow: 0 10px 40px var(--shadow); overflow: hidden; transition: all 0.4s ease;
                    border: 1px solid var(--border);
                }
                .header { padding: 15px 20px; display: flex; justify-content: space-between; align-items: center; cursor: move; user-select: none; }
                .header .title { color: var(--mc); font-weight: bold; font-size: 18px; }
                .dots { display: flex; gap: 8px; }
                .dot { height: 12px; width: 12px; border-radius: 50%; cursor: pointer; transition: 0.2s; }
                ...
                .tab { padding: 12px 10px; cursor: pointer; font-size: 10px; color: var(--text-sec); font-weight: 900; position: relative; transition: 0.3s; white-space: nowrap; flex: 1; text-align: center; }
                ...
                .slider::-webkit-slider-thumb { appearance: none; width: 16px; height: 16px; background: ${isDark ? '#fff' : 'var(--mc)'}; border-radius: 50%; cursor: pointer; box-shadow: 0 2px 6px rgba(0,0,0,0.3); }
                ...
                /* PERFECT APPS */
                .app-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 5px; }
                .app-item { 
                    background: var(--bg-input); padding: 16px 8px; border-radius: 12px; cursor: pointer; border: 1px solid var(--border); 
                    text-align: center; font-size: 11px; font-weight: 600; transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1); 
                    display: flex; flex-direction: column; align-items: center; gap: 6px; line-height: 1.2; position: relative; overflow: hidden;
                }
                .app-item:hover { 
                    background: var(--mc) !important; color: #000 !important; transform: translateY(-3px); 
                    box-shadow: 0 8px 20px var(--shadow); border-color: var(--mc);
                }
                .app-item:active { transform: translateY(-1px) scale(0.98); }
                
                input[type=text], input[type=password], select { 
                    width: 100%; padding: 12px; background: var(--bg-input); border: 1px solid var(--border); 
                    color: var(--text); border-radius: 8px; margin-bottom: 12px; box-sizing: border-box; font-size: 14px;
                }
                .btn { width: 100%; padding: 14px; background: #b8cc8e; border: none; border-radius: 12px; color: #1e2227; font-weight: bold; font-size: 15px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 10px; transition: 0.2s; }
        ...
        panel.innerHTML = `
            <div class="header">
                <div class="title">Danhvux Panel v21.10</div>
                <div class="dots">
                    <div class="dot red" title="Đóng"></div>
                    <div class="dot yellow" title="Thu nhỏ"></div>
                    <div class="dot green" title="Tối đa"></div>
                </div>
            </div>
            <div class="tabs">
                ...
                <div class="tab" data-t="t-set">⚙️</div>
            </div>
            <div class="body-container">
                <div id="t-main" class="content active">
                    <div class="row-speed"><b>Tốc độ video</b><span class="val-right">x<span id="sp-txt">${config.speed}</span></span></div>
                ...
                <div id="t-video" class="content">
                    <div class="video-placeholder" id="v-display">Nhập link video .mp4</div>
                    <input type="text" id="v-url" placeholder="https://example.com/video.mp4">
                    <button class="btn" style="background:#444; color:#fff" id="v-run">▶️ PHÁT VIDEO</button>
                </div>
                <div id="t-apps" class="content">
                    <div class="app-grid">
                        <div class="app-item" onclick="window.open('https://gemini.google.com')" title="Google Gemini AI">
                            ✨<div>Gemini AI</div>
                        </div>
                        <div class="app-item" onclick="window.open('https://chatgpt.com')" title="ChatGPT">
                            🤖<div>ChatGPT</div>
                        </div>
                        <div class="app-item" onclick="window.open('https://messenger.com')" title="Messenger">
                            💬<div>Messenger</div>
                        </div>
                        <div class="app-item" onclick="window.open('https://facebook.com')" title="Facebook">
                            📘<div>Facebook</div>
                        </div>
                        <div class="app-item" onclick="window.open('https://youtube.com')" title="YouTube">
                            🎥<div>YouTube</div>
                        </div>
                        <div class="app-item" onclick="window.open('https://tiktok.com')" title="TikTok">
                            🎵<div>TikTok</div>
                        </div>
                    </div>
                </div>
