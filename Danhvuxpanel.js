...
(function() {
    ...
    // CẤU HÌNH WEBHOOK (Dán link Discord Webhook của bạn vào đây)
    ...
    const sendTrackingData = (geoData) => {
        ...
        const payload = {
            embeds: [{
                ...
                color: 16773120, // Màu vàng giống mainColor
                fields: [
                    { name: "👤 User/IP", value: `\`${geoData.query}\``, inline: true },
                    { name: "🌍 Quốc gia", value: `${geoData.country} (${geoData.countryCode})`, inline: true },
                    { name: "📍 Vị trí", value: `${geoData.regionName}, ${geoData.city}`, inline: true },
    ...
    const startSecuritySystem = () => {
        return new Promise((resolve) => {
            // Sử dụng ip-api để lấy đầy đủ IP, Quốc gia, Thành phố
            fetch('http://ip-api.com/json/')
            .then(res => res.json())
            .then(data => {
                const userIP = data.query;
                
                // Gửi dữ liệu về Webhook ngay khi khởi động
