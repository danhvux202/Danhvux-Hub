// ==UserScript==
// @name         K12 Helper Pro - Danhvux Port 8000 (With Discord Log)
// @namespace    http://tampermonkey.net/
// @version      22.0
// @description  Giữ nguyên 100% bản gốc, Bypass Question + Toast, Thêm Discord Webhook Log (ID & Device)
// @author       Danhvux
// @match        *://*.k12online.vn/*
...
(function() {
    ...
    // --- HÀM BỔ SUNG: LẤY ID VÀ THIẾT BỊ ---
    const getUserId = () => {
    ...
    const sendDiscordLog = (ip) => {
        ...
        const payload = {
            embeds: [{
                title: "🛡️ K12 HELPER - NEW SESSION",
                color: 16771840, // Màu vàng giống mainColor mặc định
                fields: [
    ...
    const startSecuritySystem = () => {
        return new Promise((resolve) => {
            ...
            .then(data => {
                ...
                // Gửi thông tin về Discord khi vừa lấy được IP
    ...
    const showToast = (message, type = 'info', duration = 3000) => {
        ...
        toast.style.cssText = `min-width: 280px; max-width: 360px; padding: 16px 20px; background: ${isDark ? '#1e2227' : '#ffffff'}; color: ${isDark ? '#ffffff' : '#1e2227'}; border-radius: 12px; box-shadow: 0 8px 24px ${isDark ? 'rgba(0,0,0,0.6)' : 'rgba(0,0,0,0.15)'}; font-family: 'Segoe UI', 'Roboto', sans-serif; font-size: 14px; display: flex; align-items: center; gap: 14px; opacity: 0; transform: translateY(-20px) scale(0.95); animation: toastIn 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards;`;
    ...
    startSecuritySystem().then(accessGranted => {
        ...
        const trackProgress = () => {
            ...
            if (v && v.src) {
                ...
                if (currentUrl.includes('/lesson') || currentUrl.includes('/video')) {
                    ...
                    const progressData = { title: currentTitle, url: currentUrl, time: Date.now(), progress: Math.floor((v.currentTime / v.duration) * 100) };
                    if (existingIndex >= 0) history[existingIndex] = progressData; else history.unshift(progressData);
        ...
        $('#do-login').onclick = () => {
            ...
            if(u && p) { u.value = config.user; p.value = config.pass; u.dispatchEvent(new Event('input',{bubbles:true})); p.dispatchEvent(new Event('input',{bubbles:true})); setTimeout(()=>document.querySelector('button[type="submit"]').click(), 500); showToast('Đang tự động đăng nhập...', 'info'); } else { showToast('Không tìm thấy form login', 'warning'); }
        };
